...
def add_student(request):
    if request.session.has_key('username') and request.session.has_key('academic_year'):
        ...
        if "GET" == request.method:
            ...
            for row in worksheet.iter_rows(min_row=2, max_row=worksheet.max_row): 
                user_name = row[5].value
                register_number = row[1].value
                register_number_exist = StudentDetail.objects.filter(register_number=register_number)
                user_exist = StudentDetail.objects.filter(username=user_name)
                if user_exist:
                    messages.success(request, user_name + " Username Already Exist")
                elif register_number_exist:
                    messages.success(request, str(register_number) + " Register Number Already Exist ")
                else:
...
def add_class_section(request):
    if request.session.has_key('username') and request.session.has_key('academic_year'):
        ...
        academic_year = request.session['academic_year']
        cursor = connection.cursor()
        cls_new = ''' SELECT school_class.class_id,school_class.class_name,school_section.section_id,school_section.section_name
        from school_class LEFT JOIN school_section ON school_class.class_id=school_section.class_id_id where
        school_class.academic_year='%s' AND school_class.school_id_id='%d' order by school_section.section_id DESC''' % (academic_year,school_id)
    ...
    if request.session.has_key('username') and request.session.has_key('academic_year'):
        ...
        if request.method == "POST":
            ...
            for i in range(0,length):
                subject_exist = Subject.objects.filter(subject_name=sub_name[i],class_id=cls_id,section_id=sec_id,school_id=school_name)
                if subject_exist:
                    messages.success(request, sub_name[i] + " Subject Already Exist")
                else:
                    sub = Subject.objects.create(subject_name=sub_name[i],class_id=cls_id,section_id=sec_id,school_id=school_name)
            if sub:
